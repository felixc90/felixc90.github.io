...
const ModelIcon = ({ url }: ModelIconProps) => {
  ...
  useEffect(() => {
    const convertToPNG = () => {
      ...
      setImageDataUrl(pngUrl); // Store the dataURL in state
    };
  ...
  return (
    <>
      ...
      {!imageDataUrl && <canvas ref={canvasRef} style={{ display: 'none' }} />}
      {imageDataUrl && <img src={imageDataUrl} style={{ width: '80px', height: '80px' }} alt="Model Preview" />}
    </>
  );
};
