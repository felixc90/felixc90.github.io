    ...
    <div className="h-4/5 flex w-full flex-col md:flex-row">
      <div className="w-full md:w-[calc(100%-8rem)] md:max-w-[50%] md:pr-12 flex-col flex justify-between">
        <div className="">
          ...
          <div className="h-full w-full lg:text-xl md:text-sm">
            <MySplitText className="my-2">{bio}</MySplitText>
            <div className="flex gap-1.5 flex-wrap">
              {skills.map((skill, i) => (
                <Button
                  key={i}
                  variant={i == activeLogo ? "filled" : "outline"}
      ...
      >
        ...
        <Rotating axis="y" speed={3 / 4}>
          {skills[activeLogo].name.toLowerCase() === "javascript" && (
            <JavaScript />
          )}
          {skills[activeLogo].name.toLowerCase() === "typescript" && (
            <TypeScript />
          )}
